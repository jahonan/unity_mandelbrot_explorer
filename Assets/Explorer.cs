...
public class Explorer : MonoBehaviour
{
    ...
    private void HandleInputs()
    {
        ...
        if(Input.GetKey(KeyCode.Q)) angle += .01f;
        if(Input.GetKey(KeyCode.E)) angle -= .01f;
    ...
    // FixedUpdate is called once per tick (not on every frame)
    void FixedUpdate()
    {
        HandleInputs();
        UpdateShader();  
